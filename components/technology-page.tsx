...
import { Arrow, Experience, FinalInvitation } from "@/components/experience";
import { ScrollCards, useFloatingProgress } from "@/components/scroll-scenes";
...
export function TechnologyPage() {
    ...
    const progressRef = useFloatingProgress<HTMLDivElement>();
    useEffect(() => {
        const bar=progressRef.current;
        const tab=bar?.children[active] as HTMLElement | undefined;
        if (!bar || !tab || !window.matchMedia("(max-width: 899px)").matches) return;
        bar.scrollTo({left:tab.offsetLeft-(bar.clientWidth-tab.offsetWidth)/2,behavior:window.matchMedia("(prefers-reduced-motion: reduce)").matches?"instant":"smooth"});
    },[active,progressRef]);
    useEffect(() => {
    ...
    return <Experience className="technology-page">
        ...
        <section id="architecture" ref={section} className="architecture-scroll" data-mode={scrollMode?"scroll":"manual"} aria-label={de?"Interaktive Architektur":"Interactive architecture"}>
            <div className="architecture-lock"><div className="architecture-layout">
                ...
                <div ref={progressRef} className="architecture-tabs mobile-floating-progress" role="tablist" aria-label={de?"Architekturschichten":"Architecture layers"}>{copy.map((step,i)=><button key={step.label} type="button" role="tab" id={`architecture-tab-${i}`} aria-controls="architecture-panel" aria-selected={i===active} tabIndex={i===active?0:-1} onClick={()=>select(i)} onKeyDown={event=>key(event,i)}><span>0{i+1}</span>{step.label}<i style={{transform:`scaleX(${scrollMode?Math.max(0,Math.min(1,progress*6-i)):i===active?1:0})`}}/></button>)}</div>
            </div></div>
        </section>
        <ScrollCards id="architecture-principles" intro={<><p className="kicker">{de?"KLARE AUFGABEN. EIN SYSTEM.":"CLEAR RESPONSIBILITIES. ONE SYSTEM."}</p><h2>{de?<>Verteilt arbeiten.<br/><span className="muted">Zusammen verstehen.</span></>:<>Work independently.<br/><span className="muted">Understand together.</span></>}</h2></>} items={(de?[
