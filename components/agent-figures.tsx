...
export function FlowPath({d,delay=0}:{d:string;delay?:number}) {
    return <g><path d={d} pathLength="1" className="figure-path figure-flow-path" data-flow-route data-flow-delay={delay}/><circle r="3.5" className="figure-flow-dot" data-flow-dot/></g>;
}
...
export function AgentFigure({ stage }: { stage: number }) {
    ...
    return <svg key={stage} className={`agent-figure figure-stage-${stage}`} viewBox="0 0 600 260" role="img" aria-label={labels[stage]}>
        ...
        {stage === 0 && <>
            ...
            <path d="M115 126H485M300 27V226" className="figure-guide" /><g className="figure-scan"><path d="M300 126V27A184 99 0 0 1 375 36Z" fill={`url(#${id}-glow)`}/><path d="M300 126V27" className="figure-guide"/></g>
            ...
            <FlowPath d="M300 126L387 63"/><circle cx="300" cy="126" r="21" className="figure-core" /><path d="M290 126h7l4-7 4 14 3-7h5" className="figure-stroke" />
        ...
        {stage === 1 && <>
            {[[110,72],[110,188],[490,72],[490,188]].map(([x,y],i) => <FlowPath key={i} d={`M${x} ${y}C${x<300?220:380} ${y},${x<300?190:410} 130,300 130`} delay={i*.1}/>)}
        ...
        {stage === 2 && <>
            ...
            <FlowPath d="M222 126H279"/><FlowPath d="M335 126H390" delay={.35}/><path d="m380 121 10 5-10 5" className="figure-stroke"/>
        ...
        {stage === 3 && <>
            {[0,1,2].map(i=><g key={i} transform={`translate(0 ${i*41})`}><path d="M172 81L300 35L428 81L300 127Z" className="figure-memory"/><FlowPath d="M172 81L300 127L428 81" delay={i*.15}/></g>)}
            <path d="M300 52V204" className="figure-guide"/><circle cx="300" cy="52" r="5" className="figure-hot"/>
            <FlowPath d="M440 71C510 73 503 214 401 223" delay={.25}/><path d="m410 216-10 7 13 3" className="figure-stroke"/>
